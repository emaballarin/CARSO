...
def main() -> None:
    parser = argparse.ArgumentParser(
        description="ResNet18+CARSO on CIFAR10 inference and comparison"
    )
    parser.add_argument(
        "--e2e",
        action="store_true",
    ...
    adversarial_classifier = PreActResNet()
    ...
    carso_machinery.repr_compressor.load_state_dict(
        th.load("../models/carso_reprcompressor_rebuffirn18_cifar10_adv.pth")
    )
    carso_machinery.dec.load_state_dict(
        th.load("../models/carso_dec_rebuffirn18_cifar10_adv.pth")
    )
